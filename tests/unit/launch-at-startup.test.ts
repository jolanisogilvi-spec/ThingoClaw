...
const {
  ...
  const testRoot = `${process.cwd().replace(/\\/g, '/')}/.tmp`;
  const setLoginItemSettingsMock = vi.fn();
  const electronAppMock = {
    isPackaged: true,
    getPath: (name: string) => (name === 'home' ? `${testRoot}/clawx-launch-startup-${suffix}` : testRoot),
    setLoginItemSettings: setLoginItemSettingsMock,
  };

  return {
    testHome: `${testRoot}/clawx-launch-startup-${suffix}`,
    electronAppMock,
    setLoginItemSettingsMock,
  };
});
...
describe('launch-at-startup integration', () => {
  ...
  it('creates and removes Linux autostart desktop entry', async () => {
    ...
    expect(content).toContain('Name=ThingoClaw');
