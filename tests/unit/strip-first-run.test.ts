...
describe('ensureClawXIdentityFile', () => {
  it('writes a default ThingoClaw identity when the workspace has none', async () => {
  ...
  it('migrates the old generated ClawX identity but preserves user-edited identities', async () => {
    const workspaceDir = join(testHome, '.openclaw', 'workspace');
    await mkdir(workspaceDir, { recursive: true });
    ...
    await writeFile(join(workspaceDir, 'IDENTITY.md'), legacyIdentity, 'utf-8');

    await ensureClawXIdentityFile(workspaceDir);

    const migrated = await readFile(join(workspaceDir, 'IDENTITY.md'), 'utf-8');
...
describe('ensureClawXDefaultIdentity', () => {
  it('creates the default workspace and seeds IDENTITY.md for startup-owned workspaces', async () => {
    ...
    await expect(readFile(join(testHome, '.openclaw', 'workspace', 'IDENTITY.md'), 'utf-8')).resolves.toContain('ThingoClaw');
  });
});
...
describe('ensureClawXContext', () => {
  it('does not wait for missing files in non-default agent workspaces', async () => {
    ...
    await expect(readFile(join(defaultWorkspace, 'AGENTS.md'), 'utf-8')).resolves.toContain('## ThingoClaw Environment');
    await expect(readFile(join(defaultWorkspace, 'TOOLS.md'), 'utf-8')).resolves.toContain('## ThingoClaw Tool Notes');
