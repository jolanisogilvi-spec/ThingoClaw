; ThingoClaw custom NSIS Installer/Uninstaller Script
;
...
Function ClawXMoveLegacyInstallDir
  ...
  IfFileExists "$R6\" 0 _clawx_legacy_move_done
    DetailPrint "Moving previous ThingoClaw installation at $R6 out of the way..."
    SetOutPath $TEMP
  ...
  _clawx_legacy_found_free_stale:
    ...
    IfErrors 0 _clawx_legacy_stale_moved
      ...
      Rename "$R6" "$R6._stale_$R8"
      IfErrors 0 _clawx_legacy_stale_moved
      DetailPrint "Removing previous ThingoClaw installation at $R6..."
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting ThingoClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ; PowerShell path-based kill may miss processes if the old ThingoClaw was installed
  ...
  _clawx_verify_closed:
    ...
    ${if} $R0 == 0
      ...
      ${endIf}
      MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION "ThingoClaw is still running and cannot be replaced safely. Please close ThingoClaw and retry installation." /SD IDCANCEL IDRETRY _clawx_verify_closed
      SetErrorLevel 2
      Quit
    ${endIf}
  ...
  ; "Can't modify ThingoClaw's files" retry loop -> "ThingoClaw 无法关闭" dialog.
  ;
  ...
  _found_free_stale:
    ...
    IfErrors 0 _stale_moved
      ...
      IfFileExists "$INSTDIR\" 0 _recreate_clean_instdir
        ...
        MessageBox MB_OK|MB_ICONEXCLAMATION "Unable to replace the previous ThingoClaw installation because files are still locked. Please close ThingoClaw and retry installation." /SD IDOK
        SetErrorLevel 2
        Quit
...
; "ThingoClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  DetailPrint "Clearing stale ThingoClaw registry entries from the opposite install scope..."
  ${if} $installMode == "all"
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with ThingoClaw's
